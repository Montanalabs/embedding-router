...
Untrusted input is a text chunk; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a text chunk
- **Closed type:** `type Decision = Route(EmbedModel) | Skip` where `EmbedModel` is closed
- **Sink / capability:** `grant routeEmbed` → `privileged { routeEmbed(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/embedding-router/demo.sh
```
...
- `embedding-router_safe.eide` — the correct design.
- `embedding-router_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
